...
fn solve_clause(
  ...
  case clause {
    ...
    types.BloomJoin(_, clauses) -> {
      list.fold(clauses, [ctx], fn(acc, c) {
        list.flat_map(acc, fn(context) {
          solve_clause(db_state, c, context, rules, as_of_tx, as_of_valid)
        })
      })
    }
...
fn solve_clause_with_derived(
  ...
  case clause {
    ...
    types.BloomJoin(_, clauses) -> {
      list.fold(clauses, [ctx], fn(acc, c) {
        list.flat_map(acc, fn(context) {
          solve_clause_with_derived(db_state, c, context, derived, as_of_tx, as_of_valid)
        })
      })
    }
    types.BloomFilter(var, filter) -> {
