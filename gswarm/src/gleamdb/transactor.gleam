...
fn handle_message(state: types.DbState, msg: Message) -> actor.Next(types.DbState, Message) {
  case msg {
    ...
    Prune(_threshold, _sovereign, reply_to) -> {
      process.send(reply_to, 0)
    ...
    RetractEntity(eid, reply_to) -> {
       ...
       case is_leader(state) {
        True -> do_handle_transact(state, facts, option.None, fact.Retract, reply_to)
        False -> {
           case global.whereis("gleamdb_leader") {
             Ok(leader_pid) -> {

               let leader_subject = process_extra.pid_to_subject(leader_pid)
               process.send(leader_subject, RetractEntity(eid, reply_to))
               actor.continue(state)
             }
             Error(_) -> do_handle_transact(state, facts, option.None, fact.Retract, reply_to)
           }
        }
       }
    }
...
fn retract_recursive_collected(state: types.DbState, eid: fact.EntityId, tx_id: Int, valid_time: Int, acc: List(fact.Datom)) -> #(types.DbState, List(fact.Datom)) {
  let retrieved = case state.ets_name {
    Some(name) -> ets_index.lookup_datoms(name <> "_eavt", eid)
    None -> index.filter_by_entity(state.eavt, eid)
  }
  let children = retrieved |> filter_active()
